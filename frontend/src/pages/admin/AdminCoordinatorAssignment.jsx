...
function AdminCoordinatorAssignment() {
  ...
  const { data: coordinators = [], isLoading: coordsLoading } = useQuery(
    ...
    {
      staleTime: 1000 * 60 * 5,
      initialData: queryClient.getQueryData(["coordinatorsWithCohorts", token]),
    },
  );
  ...
  const { data: cohorts = [], isLoading: cohortsLoading } = useQuery(
    ...
    {
      staleTime: 1000 * 60 * 5,
      initialData: queryClient.getQueryData(["cohorts", token]),
    },
  );
  ...
  const { data: assignments = [], isLoading: assignmentsLoading } = useQuery(
    ...
    {
      staleTime: 1000 * 60 * 5,
      initialData: queryClient.getQueryData(["assignments", token]),
    },
  );

  const loading =
    coordsLoading || coursesLoading || cohortsLoading || assignmentsLoading;
  ...
  const sortAssignments = (arr) =>
    (arr || []).slice().sort((a, b) => {
      if ((a?.courseId || 0) !== (b?.courseId || 0))
        return (a?.courseId || 0) - (b?.courseId || 0);
      if ((a?.cohortId || 0) !== (b?.cohortId || 0))
        return (a?.cohortId || 0) - (b?.cohortId || 0);
  ...
  const assignCohortMutation = useMutation(
    async ({ coordinatorId, courseId, cohortId }) => {
      const res = await fetch("/api/admin/assign-coordinator", {
        method: "POST",
        headers: { ...headers, "Content-Type": "application/json" },
        body: JSON.stringify({ coordinatorId, courseId, cohortId }),
      });
    ...
    },
    {
      onMutate: async (vars) => {
        ...
        const previousAssignments =
          queryClient.getQueryData(["assignments", token]) || [];
        const previousCohorts =
          queryClient.getQueryData(["cohorts", token]) || [];
        const previousCoordinators =
          queryClient.getQueryData(["coordinatorsWithCohorts", token]) || [];
        ...
        queryClient.setQueryData(["assignments", token], (prev = []) => {
          const without = (prev || []).filter(
            (p) => p.cohortId !== Number(vars.cohortId),
          );
          return [...without, tempAssignment];
        });

        // update cohort's coordinator locally
        queryClient.setQueryData(["cohorts", token], (prev = []) =>
          (prev || []).map((c) =>
            c.id === Number(vars.cohortId)
              ? {
                  ...c,
                  coordinator: {
                    id: Number(vars.coordinatorId),
                    name: coordObj?.name,
          ...
          (prev = []) => {
            ...
            return arr.map((c) => {
              if (Number(c.id) === Number(vars.coordinatorId)) {
                const exists = (c.cohorts || []).some(
                  (co) => co.id === Number(vars.cohortId),
                );
                if (exists) return c;
                const course = (previousCourses || []).find(
                  (cr) => Number(cr.id) === Number(vars.courseId),
                );
                const cohortName =
                  (
                    previousCohorts.find(
                      (cc) => Number(cc.id) === Number(vars.cohortId),
                    ) || {}
                  ).name || "";
                const cohortBrief = {
                  id: Number(vars.cohortId),
                  name: cohortName,
  ...
  const unassignCohortMutation = useMutation(
    async ({ assignmentId }) => {
    ...
    },
    {
      onMutate: async ({ assignmentId, cohortId, coordinatorId }) => {
        ...
        queryClient.setQueryData(["assignments", token], (prev = []) =>
          (prev || []).filter(
            (p) => p.id !== assignmentId && p.cohortId !== cohortId,
          ),
        );
        queryClient.setQueryData(["cohorts", token], (prev = []) =>
          (prev || []).map((c) =>
        ...
        queryClient.setQueryData(
          ...
          (prev = []) =>
            (prev || []).map((c) =>
              Number(c.id) === Number(coordinatorId)
                ? {
                    ...c,
                    cohorts: (c.cohorts || []).filter(
                      (co) => co.id !== Number(cohortId),
                    ),
                  }
                : c,
            ),
  ...
  const handleCoordinatorChange = (courseId, cohortId, newValue) => {
    ...
    if (!newValue) {
      ...
      const isTemp =
        ...
        String(existing.id).startsWith("tmp-");
      if (isTemp) {
        // remove temp assignment from cache
        queryClient.setQueryData(["assignments", token], (prev = []) =>
          (prev || []).filter((p) => p.cohortId !== cohortId),
        );
        ...
        queryClient.setQueryData(
          ...
          (prev = []) =>
            (prev || []).map((c) => ({
              ...c,
              cohorts: (c.cohorts || []).filter((co) => co.id !== cohortId),
            })),
        );
        setInfo("Usunięto przypisanie");
        return;
        ...
        {loading ? (
          ...
          <>
            {sortedCourses.map((course) => {
              const cohortsForCourse = sortedCohorts.filter((c) =>
                c.course
                    ...
                    <table className="assignments-table">
                      ...
                      <tbody>
                        {cohortsForCourse.map((cohort) => {
                          const assigned = sortedAssignments.find(
